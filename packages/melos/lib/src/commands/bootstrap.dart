...
const melosGeneratedLineMessage =
    '# THIS LINE IS AUTOGENERATED - TO UPDATE MODIFY "melos.yaml"';
...
mixin _BootstrapMixin on _CleanMixin {
          ...
          if (bootstrapCommandConfig.environment != null ||
            ...
            await Stream.fromIterable(filteredPackages).parallel((package) {
              return _setSharedDependenciesForPackage(
                package,
                generateSyncMessage: bootstrapCommandConfig.generateSyncMessage,
  ...
  Future<void> _setSharedDependenciesForPackage(
    Package package, {
    required bool generateSyncMessage,
    ...
    final pubspecManager = pubspec.PubSpec.load(
  ...
  bool _updateEnvironment({
    ...
    print('W: $workspaceUnParsedYaml');
    print('P: $packageUnParsedYaml');
  ...
  int _updateDependencies({
    required YamlEditor pubspecEditor,
    required pubspec.PubSpec pubspecManager,
    ...
    for (final entry in dependenciesToUpdate) {
      ...
      // pubspecManager.dependencies..remove(entry.key)..add(entry.value),
      pubspecEditor.update(
